...
g_transforms = albumentations.Compose([
        ...
        NumpyBatch(albumentations.Compose([
            albumentations.ElasticTransform(p=0.5, alpha=150, alpha_affine=1, sigma=10),
            albumentations.ShiftScaleRotate(p=0.8, rotate_limit=25),
        ])),
        ToTensor(device),
    ])
...
for i in range(100000):
    ...
    if i % 10000 == 0 and i > 0:
        torch.save(
            ...
            f'{Paths.default.models()}/cardio_brule_unsup_2_{str(i + starting_model_number).zfill(6)}.pt',
        )
