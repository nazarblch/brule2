...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self, x):
        ...
        if self.mu_init is not None:
            ...
            self.mu = torch.nn.Parameter(torch.randn(1, self.n_components, self.n_features, B, device=device), requires_grad=False)
        ...
        if self.var_init is not None:
            ...
            self.var = torch.nn.Parameter(torch.ones(1, self.n_components, self.n_features, B, device=device), requires_grad=False)

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.ones(1, self.n_components, 1, B, device=device), requires_grad=False).fill_(1./self.n_components)
    ...
    def bic(self, x):
        ...
        bic = -2. * self.__score(x, sum_data=False).mean(dim=[0, 1, 2]) * n + free_params * np.log(n)
    ...
    def _e_step(self, x):
        ...
        return torch.mean(log_prob_norm, dim=[0, 1, 2]), log_resp
    ...
    def _m_step(self, x, log_resp):
        ...
        var = var.mean(dim=2, keepdim=True).repeat(1, 1, var.shape[2], 1)
    ...
    def __score(self, x, sum_data=True):
        ...
        if sum_data:
            return per_sample_score.sum(dim=[0, 1, 2])
        else:
    ...
    def __update_mu(self, mu):
        ...
        assert mu.size() in [(self.n_components, self.n_features), (1, self.n_components, self.n_features, mu.shape[-1])], "Input mu does not have required tensor dimensions (%i, %i) or (1, %i, %i)" % (self.n_components, self.n_features, self.n_components, self.n_features)

        if mu.size() == (self.n_components, self.n_features):
            self.mu = mu.unsqueeze(0)
        elif mu.size() == (1, self.n_components, self.n_features, mu.shape[-1]):
            self.mu.data = mu
    ...
    def __update_var(self, var):
        ...
        assert var.size() in [(self.n_components, self.n_features), (1, self.n_components, self.n_features, var.shape[-1])], "Input var does not have required tensor dimensions (%i, %i) or (1, %i, %i)" % (self.n_components, self.n_features, self.n_components, self.n_features)

        if var.size() == (self.n_components, self.n_features):
            self.var = var.unsqueeze(0)
        elif var.size() == (1, self.n_components, self.n_features, var.shape[-1]):
            self.var.data = var
    ...
    def __update_pi(self, pi):
        ...
        assert pi.size() in [(1, self.n_components, 1, pi.shape[-1])], "Input pi does not have required tensor dimensions (%i, %i, %i)" % (1, self.n_components, 1)

        self.pi.data = pi


class MaskToMixtureGPU(ProbabilityMeasureFabric):

    def __init__(self, hw_size: int, n_components=100):

        super().__init__(hw_size)
        self.n_components = n_components
    ...
    def forward(self, mask: torch.Tensor, border=0.5):
        ...
        gmm = GaussianMixture(
            n_components=self.n_components, n_features=2
        ).cuda()
...
class MaskToMixture(ProbabilityMeasureFabric):

    def __init__(self, hw_size: int, n_components=100):

        super().__init__(hw_size)
        self.n_components = n_components
        # self.par = Parallel(n_jobs=16)
    ...
    def forward(self, mask: Tensor, border=0.5):
        ...
        probabilymeasurelist = [self._from_one_mask(mask[i], border) for i in range(len(mask))]
        ...
        def compute(coord, n_components):
            ...
            if coord.shape[0] < n_components:
                return coord[np.newaxis, :, :], np.ones((1, coord.shape[0])) / coord.shape[0]

            gmm = mixture.GaussianMixture(
                n_components=n_components, covariance_type='spherical', max_iter=100, tol=1e-4
            )
            gmm.fit(coord)
            return gmm.means_[np.newaxis, :, :], gmm.weights_[np.newaxis, :]

        means_and_probs = [compute(mes.coord[0].numpy(), self.n_components) for mes in probabilymeasurelist]

        means = [torch.from_numpy(mm[0]).type(torch.float32) for mm in means_and_probs]
        probs = [torch.from_numpy(mm[1]).type(torch.float32) for mm in means_and_probs]
